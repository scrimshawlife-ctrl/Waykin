...
public struct AudioExperienceLayer {
    ...
    public static func map(behavior: CompanionBehaviorState, intensity: Double = 0.45) -> AudioCue? {
        ...
        switch behavior {
        case .drawNear:
            return AudioCue(kind: .companionNear, intensity: intensity, spatialBias: .left, priority: 2, cooldownGroup: "companion", shouldFade: true, debugLabel: label)
        case .lead:
            return AudioCue(kind: .companionAhead, intensity: intensity, spatialBias: .center, priority: 2, cooldownGroup: "companion", shouldFade: true, debugLabel: label)
        case .rest, .observe:
            return AudioCue(kind: .quietShift, intensity: min(intensity, 0.4), spatialBias: .center, priority: 1, cooldownGroup: "ambient", shouldFade: true, debugLabel: label)
        case .celebrate:
            return AudioCue(kind: .bondMotif, intensity: intensity, spatialBias: .center, priority: 5, cooldownGroup: "bond", shouldFade: true, debugLabel: label)
        case .follow, .idle:
            return nil
        }
    }
    ...
    public static func map(arPresentation raw: String, intensity: Double = 0.45) -> AudioCue? {
        ...
        case "celebrate":
            return AudioCue(kind: .bondMotif, intensity: intensity, spatialBias: .center, priority: 5, cooldownGroup: "bond", shouldFade: true, debugLabel: label)
        case "alert":
            return AudioCue(kind: .pursuitPressure, intensity: min(intensity, 0.7), spatialBias: .behind, priority: 4, cooldownGroup: "pursuit", shouldFade: false, debugLabel: label)
        case "investigate":
            return AudioCue(kind: .quietShift, intensity: min(intensity, 0.4), spatialBias: .center, priority: 1, cooldownGroup: "ambient", shouldFade: true, debugLabel: label)
        case "follow", "idle":
            return nil
