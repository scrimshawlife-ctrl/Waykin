...
Last updated: 2026-07-29 (**FREEZE MERGED** — #249; device indoor next; no redesign until device honesty)
...
|---|---|---|---|
| **Freeze: docs re-baseline + laptop receipt** | Docs / eng | **Merged** — [#249](https://github.com/scrimshawlife-ctrl/Waykin/pull/249) (merge SHA `d8c0620`) | [CONTINUATION_PLAN.md](../design/CONTINUATION_PLAN.md) · [POST_EMBER_FOX_BASELINE_20260729T190249Z_1378307.md](../design/receipts/POST_EMBER_FOX_BASELINE_20260729T190249Z_1378307.md) |
| Indoor Ember Fox smoke | Human device | **Next** — `git pull` + install **current main HEAD**; fill tip-bound indoor receipt; visual gold = Prabu IMG_2534 | [INDOOR_AR_HYBRID_SMOKE.md](../design/INDOOR_AR_HYBRID_SMOKE.md) · [DEVICE_MESH_REFERENCE_PRABU_IMG_2534.md](../design/receipts/DEVICE_MESH_REFERENCE_PRABU_IMG_2534.md) · scaffold [INDOOR_AR_HYBRID_SMOKE_20260729T191500Z_7df3a16_PENDING.md](../design/receipts/INDOOR_AR_HYBRID_SMOKE_20260729T191500Z_7df3a16_PENDING.md) |
...
| AR redesign docs recovery | Docs lane | **Landing via [#251](https://github.com/scrimshawlife-ctrl/Waykin/pull/251)** — recovered SUPPORTING docs on freeze main; supersedes stale [#245](https://github.com/scrimshawlife-ctrl/Waykin/pull/245). After merge: treat recovery **complete**; close #245 | SUPPORTING only; no Phase 0 binding edits |
...
| Field | Value |
|---|---|
| Live `main` tip | **Do not pin a fixed short SHA here after every docs merge.** Run `git checkout main && git pull --ff-only && git rev-parse HEAD` and record that full SHA on the install/receipt |
| Freeze merge (#249) | `d8c062013fea4e0e0fc7fe661b050f12135d9163` (`d8c0620`) — docs/evidence freeze; **base lineage**, not forever the live tip after later merges |
| Code lineage (app/mesh binary) | Unchanged from `7df3a169ede507ce54469330318f66c4603f8c3d` (`7df3a16`) — Ember Fox #246 + pressure #248 |
| Install / device / #247 SHA rule | Install **current main HEAD after pull**; record exact `git rev-parse HEAD` on the device build. Minimum freeze base: ≥ `d8c0620` |
...
| Open PRs (non-mesh) | #245 (docs redesign, supporting), #244 (license), board sync — treat as separate lanes |
...
1. `git checkout main && git pull --ff-only` → record **`git rev-parse HEAD`** (must be ≥ freeze merge `d8c0620`).
2. **Connect iPhone** → install Debug of **that HEAD** → indoor Ember Fox smoke (I1–I14; match Prabu visual ref) → `evidence_class: OBSERVED` (note indoor). Prefer regenerating the PENDING receipt with `scripts/indoor_ar_smoke_prep.sh` so the scaffold pins the install SHA.
...
```text
Freeze merge (#249): d8c0620 · companion: Ember Fox (MESHY_EMBER_FOX_WALK_V1) · code lineage 7df3a16
Live install tip: git pull + git rev-parse HEAD (never a remembered short alone)
